...
import { createTestAccessToken } from "../lib/chat-client.mjs";

test("Supabase: custom auth register succeeds", async () => {
  const env = loadTestEnv();
  requireEnv(env, ["supabaseUrl"]);

  const token = await createTestAccessToken(env.supabaseUrl);
